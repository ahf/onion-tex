---
title: Anti-censorship with Tor
date: November 10, 2019
institute: Freedom not Fear
author:
...
\begin{tikzpicture}
    ...
    %% Encrypted data.
    \node[] at (0, 0.5) {\tiny{\texttt{\ldots RG9uJ3QgdXNlIGIgZW5jcnlwdGlvbi4\ldots}}};
...
\centering\large Please email the stop page of your country to \href{mailto:ahf@torproject.org}{ahf@torproject.org}.
...
\begin{tikzpicture}[overlay,scale=1.32]
    ...
    \node[font=\footnotesize\bfseries, align=center] at (-4, 2.5) {Censored Network};
    ...
    \draw[<-, thick, OnionDarkPurple!80] (-2.5, 0) -- (-1.05, 0);
...
\begin{tikzpicture}[overlay,scale=1.32]
    ...
    %% Adversary.
    \node[maninblack, monitor, minimum size=0.6cm] (adversary) at (-2, 1.5) {};
    \node[ellipse callout, draw, yshift=0.3cm, callout absolute pointer={(adversary.mouth)}, font=\scriptsize\bfseries, align=center, fill=white] at (-1.4, 1.75) {?};

    %% Adversary monitors Alice.
    \node[wiretap] (wiretap) at (-1.05, 0) {};
    \path[thick, red!40] (adversary) edge (wiretap);
...
\begin{tikzpicture}[overlay,scale=1.32]
    ...
    %% Our adversary connects to B2.
    \path[<->, thick, OnionDarkPurple!80, bend left=20, shorten >= 0.4cm, shorten <= 0.4cm] (adversary.east) edge (3.5, 0.8);
...
\begin{tikzpicture}[overlay,scale=1.32]
    ...
    %% Alice connects to B2
    \draw[<-, thick, OnionDarkPurple!80] (-3.0, 0) -- (-1.05, 0);
...
\begin{tikzpicture}[overlay,scale=1.32]
    ...
    %% Connection between Alice and the Webserver box (behind the HTTP+TLS box).
    \path[<->, thick, shorten <= 0.8cm, shorten >= 0.4cm, bend right=10] (alice.east) edge (webserver);
    ...
    \path[<->, thick, bend right, shorten <= 0.8cm, shorten >= 0.4cm, bend right=10] (alice) edge (dns query);
